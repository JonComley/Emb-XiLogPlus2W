 ...
 * Prototypes
 *****************************************************************************/
#define Delayms(X) TIM_delay_ms(X)
    ...
    else
    {
        for(index = 0; index < gMediaSectorSize; index++)      //Reads in a sector of data (512 bytes)
        {
            if(buffer != NULL)
            {
				...
				#else
	                SPIBUF = 0xFF;
					if (SPIENABLE)				// avoid infinite loops
						while (!SPISTAT_RBF);
	                buffer[index] = SPIBUF;
				#endif
            }
...
Dog awful function replaced by TIM_delay_ms. MA
void Delayms(BYTE milliseconds)
{
    BYTE    ms;
    DWORD   count;
...
//#ifdef NEEDED	// Replaced by cfs_init_sd_card & CFS_sd_card_ready
...
MEDIA_INFORMATION *  MDD_SDSPI_MediaInitialize(void)
{
    ...
    //than 400kHz. Initialize SPI port to slower than 400kHz
		    ...
		    timeout = (WORD)(GetSystemClock() / 400000);
