...
********************************************************************
 ...
 Change History:
  ...
  V4.06   gLastFreeCluster added for faster creation of new files and avoidance of file system corruption
...
DWORD	gLastFreeCluster;			// Added by MA May 2014
...
BYTE Write_File_Entry( FILEOBJ fo, WORD * curEntry)
{
    ...
    offset2  = (BYTE)(*curEntry / (dsk->sectorSize/32));
...
CETYPE CreateFirstCluster(FILEOBJ fo)
{
    ...
    if((error = FILECreateHeadCluster(fo,&cluster)) == CE_GOOD)
    {
       ...
       dir->DIR_FstClusHI = (WORD)TempMsbCluster;
...
DWORD FATfindEmptyCluster(FILEOBJ fo)
{
    ...
    disk = fo->dsk;
    //c = fo->ccls;				// REPLACED for speedup MA May 2014
	c = gLastFreeCluster;
	...
	do
    {
        ...
        if (value == ClusterFailValue)
			return 0;

        // check next cluster in FAT
        if ((++c >= disk->maxcls + 1) || (value == EndClusterLimit))
            c = 2;					// re-start from top

        // check if full circle done, disk full
        if (c == curcls)
			return 0;

	} while (c != 0);				// scanning for an empty cluster
...
void FSGetDiskProperties(FS_DISK_PROPERTIES* properties)
{
    ...
    if(properties->new_request == TRUE)
    {
        ...
        properties->results.sector_size = (WORD)properties->disk->sectorSize;
...
/******************************************************************************
** Function:	Read line n from file
**
** Notes:		Returns string length, or -1 if past end-of-file.
*/
int FS_read_line(FSFILE *stream, int line_number, char *ptr, size_t max_bytes)
{
	...
	FSfseek(stream, 0, 0);		// start at beginning of file
    dsk    = (DISK *)stream->dsk;
    pos    = stream->pos;
    seek    = stream->seek;
	...
	if (line_number > 0)		// first line is line 1
		line_number--;
    ...
    //loop reading bytes until start of correct line found, then copy line into buffer
	i = 0;
	do
    {
        if( seek == stream->size )
        ...
        seek++;
        readCount++;
		if (line_number > 0)	// haven't found the right line yet
		{
			if (*(dsk->buffer + pos++) == '\n')
				line_number--;
		}
		else					// copy byte from buffer into output string
		{
			*ptr = *(dsk->buffer + pos++);
			if ((*ptr == '\r') || (*ptr == '\n') || (i >= (int)max_bytes))
				break;
            ...
            if (test != CE_GOOD)
            {
                if (test == CE_FAT_EOF)
                {
                    ...
                    else
                    {
                        ...
                        stream->pos = (WORD)dsk->sectorSize;
...
DWORD WriteFAT (DISK *dsk, DWORD ccls, DWORD value, BYTE forceWrite)
{
    ...
    if (dsk->type == FAT32)  // Refer page 16 of FAT requirement.
    {
        ...
        RAMwrite (gFATBuffer, p+2, (BYTE)((value & 0x00ff0000) >> 16));
        RAMwrite (gFATBuffer, p+3, (BYTE)((value & 0x0f000000) >> 24));   // the MSB nibble is supposed to be "0" in FAT32. So mask it.
    }
    else
...
#endif
    {
        if (dsk->type == FAT16)
        {
            RAMwrite (gFATBuffer, p, (BYTE)value);            //lsB
        ...
        while (!dirCleared)
        {
            ...
            else
            {
                ...
                if (subDirDepth != 0)
                {
                    ...
                    // Get the cluster
                    handle2 = (WORD)GetFullClusterNumber(entry); // Get complete cluster number.
...
int FindFirst (const char * fileName, unsigned int attr, SearchRec * rec)
{
    ...
    if (result == CE_GOOD)
    {
        ...
        rec->attributes = (unsigned char)fo->attributes;
